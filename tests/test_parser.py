...
class ParserTests:
    ...
    def test_forward(self):
        ...
        assert self.interpreter.run(self.parser.run(forward_alias_input)) == forward_response

    def test_backward(self):
        backward_input = "backward 100"
        backward_alias_input = "bk 100"
        backward_response = {"commands": [{"name": "backward", "value": 100}]}
        assert self.interpreter.run(self.parser.run(backward_input)) == backward_response
        assert self.interpreter.run(self.parser.run(backward_alias_input)) == backward_response
    ...
    def test_right(self):
        ...
        assert self.interpreter.run(self.parser.run(right_alias_input)) == right_response

    def test_showturtle(self):
        showturtle_input = "showturtle"
        showturtle_alias_input = "st"
        showturtle_response = {"commands": [{"name": "showturtle"}]}
        assert self.interpreter.run(self.parser.run(showturtle_input)) == showturtle_response
        assert self.interpreter.run(self.parser.run(showturtle_alias_input)) == showturtle_response

    def test_hideturtle(self):
        hideturtle_input = "hideturtle"
        hideturtle_alias_input = "ht"
        hideturtle_response = {"commands": [{"name": "hideturtle"}]}
        assert self.interpreter.run(self.parser.run(hideturtle_input)) == hideturtle_response
        assert self.interpreter.run(self.parser.run(hideturtle_alias_input)) == hideturtle_response
    ...
    def test_penup(self):
        ...
        assert self.interpreter.run(self.parser.run(penup_alias_input)) == penup_response
    ...
    def test_pendown(self):
        ...
        assert self.interpreter.run(self.parser.run(pendown_alias_input)) == pendown_response
