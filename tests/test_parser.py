...
class ParserTests:
    ...
    def test_forward(self):
        ...
        assert self.interpreter.run(self.parser.run(forward_input)) \
            == forward_response
        assert self.interpreter.run(self.parser.run(forward_alias_input)) \
            == forward_response
    ...
    def test_backward(self):
        ...
        assert self.interpreter.run(self.parser.run(backward_input)) \
            == backward_response
        assert self.interpreter.run(self.parser.run(backward_alias_input)) \
            == backward_response
    ...
    def test_left(self):
        ...
        assert self.interpreter.run(self.parser.run(left_input)) \
            == left_response
        assert self.interpreter.run(self.parser.run(left_alias_input)) \
            == left_response
    ...
    def test_right(self):
        ...
        assert self.interpreter.run(self.parser.run(right_input)) \
            == right_response
        assert self.interpreter.run(self.parser.run(right_alias_input)) \
            == right_response
    ...
    def test_showturtle(self):
        ...
        assert self.interpreter.run(self.parser.run(showturtle_input)) \
            == showturtle_response
        assert self.interpreter.run(self.parser.run(showturtle_alias_input)) \
            == showturtle_response
    ...
    def test_hideturtle(self):
        ...
        assert self.interpreter.run(self.parser.run(hideturtle_input)) \
            == hideturtle_response
        assert self.interpreter.run(self.parser.run(hideturtle_alias_input)) \
            == hideturtle_response
    ...
    def test_penup(self):
        ...
        assert self.interpreter.run(self.parser.run(penup_input)) \
            == penup_response
        assert self.interpreter.run(self.parser.run(penup_alias_input)) \
            == penup_response
    ...
    def test_pendown(self):
        ...
        assert self.interpreter.run(self.parser.run(pendown_input)) \
            == pendown_response
        assert self.interpreter.run(self.parser.run(pendown_alias_input)) \
            == pendown_response
